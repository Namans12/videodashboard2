...
def parse_fraction(value: Any) -> float | None:
    ...
    if "/" in text:
        ...
        numerator = coerce_float(numerator_text)
        denominator = coerce_float(denominator_text)
...
def get_mediainfo_tracks(data: dict[str, Any] | None, track_type: str) -> list[dict[str, Any]]:
    ...
    try:
        return [track for track in data["media"]["track"] if track.get("@type") == track_type]
    except (KeyError, TypeError):
        return []
...
def get_container_name(general_track: dict[str, Any] | None, ffprobe_data: dict[str, Any] | None) -> str:
    ...
    if ffprobe_data:
        format_name = normalize_text(ffprobe_data.get("format", {}).get("format_long_name", ""))
        if format_name:
            return format_name
...
def get_container_short_name(container_name: str) -> str:
    ...
    if "mp4" in text or "mpeg-4" in text:
        return "MP4"
    if "mpeg-ts" in text or "transport stream" in text:
        return "TS"
    if "m2ts" in text or "bdav" in text:
        return "M2TS"
    if "hevc" in text:
        return "RAW HEVC"
    return "Unknown"
...
def get_resolution(video_track: dict[str, Any] | None, ffprobe_video: dict[str, Any] | None) -> str:
    ...
    if not width and video_track:
        width = video_track.get("Width")
    if not height and video_track:
        height = video_track.get("Height")

    width_value = coerce_float(width)
    height_value = coerce_float(height)
...
def get_frame_rate(video_track: dict[str, Any] | None, ffprobe_video: dict[str, Any] | None) -> str:
    ...
    if ffprobe_video:
        frame_rate = parse_fraction(ffprobe_video.get("avg_frame_rate")) or parse_fraction(ffprobe_video.get("r_frame_rate"))
...
def get_pixel_format(video_track: dict[str, Any] | None, ffprobe_video: dict[str, Any] | None) -> str:
    ...
    if video_track:
        ...
        if chroma and bit_depth:
            return f"{chroma}, {bit_depth}-bit"
        if chroma:
            return chroma
...
def get_color_range(video_track: dict[str, Any] | None, ffprobe_video: dict[str, Any] | None) -> str:
    if ffprobe_video:
        range_value = normalize_text(ffprobe_video.get("color_range", ""))
        if range_value:
            return range_value

    if video_track:
        range_value = normalize_text(video_track.get("colour_range", video_track.get("ColorRange", "")))
        if range_value:
            return range_value
...
def get_mastering_display(video_track: dict[str, Any] | None) -> str:
    if not video_track:
        return "Unknown"
    value = normalize_text(video_track.get("MasteringDisplay_Luminance", ""))
...
def get_cll_summary(video_track: dict[str, Any] | None) -> str:
    ...
    if max_cll and max_fall:
        return f"MaxCLL {max_cll} nits / MaxFALL {max_fall} nits"
    if max_cll:
        return f"MaxCLL {max_cll} nits"
    if max_fall:
        return f"MaxFALL {max_fall} nits"
    return "Unknown"
...
def get_bitrate_mbps(video_track: dict[str, Any] | None, ffprobe_video: dict[str, Any] | None) -> float:
    candidates: list[Any] = []
...
def build_profile_label(base_profile: str, video_track: dict[str, Any] | None, ffprobe_video: dict[str, Any] | None) -> str:
    if base_profile == "None":
        return "None"

    compatibility_text = ""
    if video_track:
        compatibility_text = f"{normalize_text(video_track.get('HDR_Format_Compatibility', ''))} {normalize_text(video_track.get('HDR_Format_Profile', ''))}".lower()
    ...
    if base_profile == "8":
        if compatibility_id == 4 or "hlg" in compatibility_text:
            return "8.4"
        if compatibility_id == 2 or "sdr" in compatibility_text:
            return "8.2"
        if compatibility_id == 1 or "hdr10" in compatibility_text:
            return "8.1"
        return "8.x"
...
def detect_layer_variant(
    ...
    Real FEL/MEL certainty is not always available from container metadata.
    """
    ...
    if "fel" in name and "mel" not in name:
        return "FEL", "Filename hint matched FEL."
    if "mel" in name and "fel" not in name:
        return "MEL", "Filename hint matched MEL."

    if dovi_side:
        compat_id = dovi_side.get("dv_bl_signal_compatibility_id")
        if compat_id is not None:
            if "profile 7" in hdr_text or "dolby vision" in hdr_text:
                return (
                    "Dual-layer (FEL/MEL unknown)",
                    f"Profile 7 dual-layer stream detected; compatibility id={compat_id} is not enough to prove FEL or MEL.",
                )

    return "Dual-layer (FEL/MEL unknown)", "Dual-layer DV detected, but the metadata available here does not prove FEL vs MEL."


def get_audio_quality_score(label: str, details: str) -> int:
    text = f"{label} {details}".lower()

    if "truehd" in text or ("atmos" in text and "truehd" in text):
        return 10
    if "dts-hd" in text or "dts x" in text or "dtsx" in text:
        return 9
    if "dts" in text:
        return 7
    if "eac3" in text or "ddp" in text or "dolby digital plus" in text:
        return 6
    if "ac3" in text or "ac-3" in text or "dolby digital" in text:
        return 5
    if "pcm" in text or "lpcm" in text:
        return 6
    if "aac" in text:
        return 3
    if "opus" in text:
...
def get_primary_audio_summary(
    ...
    if mediainfo_audio_tracks:
        primary = next(
            (track for track in mediainfo_audio_tracks if lower_text(track.get("Default", "")) == "yes"),
...
def get_container_compatibility(container_short: str) -> str:
    if container_short in {"MKV", "MP4"}:
        return "Good"
    if container_short in {"M2TS", "TS"}:
        return "Okay"
    if container_short == "RAW HEVC":
        return "Poor"
    return "Unknown"
...
def tv_compatibility_heuristic(
    ...
    if "hevc" not in codec_text and "h.265" not in codec_text:
        return {
            "profile_supported": "No",
            "el_usable": "No",
            "container_compatibility": container_fit,
            "note": "Primary video is not HEVC, so DV playback is not a sensible target.",
        }

    if base_profile in {"5", "8.1", "8.2", "8.4"}:
        profile_supported = "Yes"
        el_usable = "No"
        note = f"Single-layer Dolby Vision profile {profile_label}; this is the safest DV shape for consumer playback."
    elif base_profile == "7":
        profile_supported = "Partial"
        if el != "Yes":
            el_usable = "No"
        elif "fel" in layer_variant.lower():
            el_usable = "Unknown"
        elif "mel" in layer_variant.lower():
            el_usable = "Partial"
        else:
            el_usable = "Partial"
        note = "Profile 7 is dual-layer and consumer playback often depends on the player path, so EL behavior is not guaranteed."
    elif base_profile == "4":
        profile_supported = "Partial"
        el_usable = "Partial"
        note = "Older dual-layer DV; workable in some chains, not the ideal modern consumer target."
    elif base_profile == "None":
        profile_supported = "No"
        el_usable = "No"
        note = "No Dolby Vision profile was detected."
    else:
        profile_supported = "Unknown"
        el_usable = "Unknown"
        note = "Dolby Vision support could not be classified confidently."
...
def score_video(
    ...
    if base_profile == "7":
        score += 42
    elif base_profile == "8.1":
        score += 30
    elif base_profile == "5":
        score += 26
    elif base_profile == "8.4":
        score += 22
    elif base_profile == "8.2":
        score += 18
    elif base_profile == "4":
        score += 14

    layer_text = layer_variant.lower()
    if layer_text.startswith("fel"):
        score += 18
    elif layer_text.startswith("mel"):
        score += 12
    elif "dual-layer" in layer_text and base_profile == "7":
        score += 8

    if bitrate_mbps > 70:
        score += 18
    elif bitrate_mbps > 45:
        score += 15
    ...
    if container_short in {"MKV", "MP4"}:
        score += 4
    elif container_short in {"M2TS", "TS"}:
        score += 2

    if source == "REMUX":
        score += 10
    elif source == "UHD BluRay":
        score += 8
    elif source == "BluRay Encode":
        score += 5
    elif source == "WEB-DL":
        score += 2
    ...
    if tv_support == "Yes":
        score += 5
    elif tv_support == "Partial":
        score += 2

    if el == "Yes" and base_profile == "7":
        score += 4
...
def score_confidence(
    ...
    if dovi_scan.get("status") in {"partial", "ok"}:
        score += 18
    elif dovi_scan.get("status") == "error":
        score -= 6
    else:
        score -= 4
    ...
    if mediainfo_data and ffprobe_data:
        ...
        ff_video = ffprobe_video
        if mi_video and ff_video:
            mi_res = f"{normalize_text(mi_video.get('Width', ''))}x{normalize_text(mi_video.get('Height', ''))}"
            ff_res = f"{ff_video.get('width', '')}x{ff_video.get('height', '')}"
...
def build_insights(
    ...
    if profile.startswith("8"):
        notes.append("Single-layer Dolby Vision with HDR-compatible delivery.")
    elif profile == "7":
        notes.append("Disc-oriented Dolby Vision profile, usually the highest-end candidate.")
    elif profile == "5":
        notes.append("Streaming-first Dolby Vision profile.")
    else:
        notes.append("No clear Dolby Vision profile detected.")

    if layer_variant.lower().startswith("fel"):
        notes.append("FEL detected by filename hint or strong metadata hint.")
    elif layer_variant.lower().startswith("mel"):
        notes.append("MEL detected by filename hint or strong metadata hint.")
    elif "dual-layer" in layer_variant.lower():
        notes.append("Dual-layer DV is present, but FEL vs MEL is not proven.")

    if bitrate_mbps >= 30:
        notes.append("Good bitrate tier for high-quality playback.")
    elif bitrate_mbps >= 15:
        notes.append("Normal WEB-DL bitrate tier.")
    else:
        notes.append("Aggressively compressed bitrate tier.")

    if dovi_scan.get("status") == "partial":
        frames = dovi_scan.get("summary", {}).get("frames", FFMPEG_SAMPLE_FRAMES)
        notes.append(f"Bounded dovi_tool pass stopped after {frames} frames on purpose.")

    notes.append(f"Primary audio: {audio_details}")
    if subtitle_count:
        notes.append(f"Subtitle tracks: {subtitle_count}")
...
def should_run_dovi_scan(video_track: dict[str, Any] | None, ffprobe_video: dict[str, Any] | None) -> tuple[bool, str]:
    codec_names = {
    ...
    if "hevc" not in codec_names and "h.265" not in codec_names and "h265" not in codec_names:
        return False, "dovi_tool was skipped because the primary video stream is not HEVC."
    ...
    if "dolby vision" not in hdr_summary:
        return False, "dovi_tool was skipped because Dolby Vision was not signaled by MediaInfo or ffprobe."

    return True, ""


def create_ffmpeg_video_sample(file_path: str, frame_limit: int) -> tuple[str | None, subprocess.CompletedProcess[str] | None]:
...
def run_dovi_partial_scan(
    ...
    if not ffmpeg_bin or not dovi_bin:
        return {
            "status": "unavailable",
            "headline": "Bounded Dolby Vision RPU scan is unavailable because ffmpeg or dovi_tool is missing.",
            "details": [],
            "summary": {},
        }
    ...
    try:
        ...
        if not sample_path:
            ...
            detail = stderr or "FFmpeg could not extract a short HEVC sample from the file."
            return {
                "status": "error",
                "headline": "FFmpeg could not prepare the Dolby Vision sample stream.",
                "details": [detail],
                "summary": {},
            }
        ...
        extract_result = run_command(
            [
                ...
                rpu_path,
                "-l",
                str(FFMPEG_SAMPLE_FRAMES),
            ],
            timeout=DOVI_TIMEOUT_SECONDS,
        )
        if command_failed(extract_result):
            stderr = extract_result.stderr.strip() if extract_result else ""
            stdout = extract_result.stdout.strip() if extract_result else ""
            detail = stderr or stdout or "dovi_tool could not extract RPU data from the sample."
            return {
                "status": "error",
                "headline": "dovi_tool could not read Dolby Vision RPU data from the sampled frames.",
                "details": [detail],
                "summary": {},
            }

        info_result = run_command([dovi_bin, "info", "-s", "-i", rpu_path], timeout=DOVI_TIMEOUT_SECONDS)
        if command_failed(info_result):
            stderr = info_result.stderr.strip() if info_result else ""
            stdout = info_result.stdout.strip() if info_result else ""
            detail = stderr or stdout or "dovi_tool could not summarize the extracted RPU data."
            return {
                "status": "error",
                "headline": "dovi_tool extracted RPU data but could not summarize it.",
                "details": [detail],
                "summary": {},
            }
        ...
        details = [f"Stopped after {frames} frames by design to avoid a full-movie dovi_tool pass."]

        if summary.get("dm_version"):
            details.append(f"DM version {summary['dm_version']}")
        if summary.get("scene_count") is not None:
            details.append(f"Scene refresh points in sample: {summary['scene_count']}")
        if summary.get("rpu_content_light_level"):
            details.append(summary["rpu_content_light_level"])

        return {
            "status": "partial",
            "headline": f"Partial Dolby Vision RPU scan completed on the first {frames} frames.",
            "details": details,
            "summary": summary,
        }
    finally:
...
def inspect_dolby_vision(
    ...
    if video_track:
        ...
        if "BL+RPU" in settings:
            bl = "Yes"
            rpu = "Yes"
            el = "No"
        elif "BL+EL+RPU" in settings:
            bl = "Yes"
            rpu = "Yes"
            el = "Yes"
    ...
    tool_names = []
    if video_track:
        tool_names.append("mediainfo")
    if side_data:
        tool_names.append("ffprobe")
    if dovi_scan.get("status") in {"partial", "ok"}:
        tool_names.append("ffmpeg")
        tool_names.append("dovi_tool")

    layer_variant, layer_reason = detect_layer_variant(filepath, base_profile, el, video_track, ffprobe_video)
...
def build_signal_facts(
    ...
    if summary.get("dm_version"):
        facts.append({"label": "CM Version", "value": str(summary["dm_version"])})
    if summary.get("frames") is not None:
        facts.append({"label": "Frames Sampled", "value": str(summary["frames"])})
    if summary.get("scene_count") is not None:
        facts.append({"label": "Scene Refreshes", "value": str(summary["scene_count"])})
    if summary.get("rpu_mastering_display"):
        facts.append({"label": "RPU Mastering Display", "value": str(summary["rpu_mastering_display"])})
    if summary.get("l2_trims"):
        facts.append({"label": "L2 Trims", "value": str(summary["l2_trims"])})
    if summary.get("l8_trims"):
        facts.append({"label": "L8 Trims", "value": str(summary["l8_trims"])})
    if summary.get("l9_mdp"):
        facts.append({"label": "L9 MDP", "value": str(summary["l9_mdp"])})
...
def build_media_facts(
    ...
    facts = [
        ...
        {
            "label": "File Size",
            "value": format_size_gb(
                general_track.get("FileSize") if general_track else ffprobe_data.get("format", {}).get("size", "")
            ),
        },
    ]
    return facts
...
def build_tool_reports(
    ...
    if mediainfo_data:
        ...
        hdr_summary = get_hdr_summary(video_track, ffprobe_video)
        if hdr_summary:
            details.append(hdr_summary)
        if audio_details != "Unknown audio":
            details.append(audio_details)
        mastering_display = get_mastering_display(video_track)
        if mastering_display != "Unknown":
            details.append(f"Mastering display: {mastering_display}")
        ...
        reports.append(
            {
                "name": "MediaInfo",
                "status": "unavailable",
                "headline": "MediaInfo was not available, so container-level HDR and audio hints are limited.",
                "details": [],
            }
        )
    ...
    if ffprobe_video:
        ...
        if side_data:
            details.append(
                ...
                f"profile {side_data.get('dv_profile', '?')}, "
        ...
        if dovi_scan.get("status") == "partial":
            details.append(f"FFmpeg copied the first {FFMPEG_SAMPLE_FRAMES} frames into a raw HEVC sample for dovi_tool.")
        ...
        reports.append(
            {
                "name": "FFmpeg / ffprobe",
                "status": "unavailable",
                "headline": "ffprobe was not available, so stream-side Dolby Vision flags could not be verified.",
                "details": [],
            }
        )
    ...
    reports.append(
        {
            ...
            "headline": dovi_scan.get("headline", "Dolby Vision RPU scan was not available."),
...
def build_recommendation(
    ...
    if tv_support["profile_supported"] == "No":
        return "Not a sensible playback target for this TV."
    if quality_score >= 80 and confidence_score >= 70:
        return "Top candidate for playback."
    if profile_label == "7" and tv_support["profile_supported"] == "Partial":
        if "fel" in layer_variant.lower():
            return "Best-looking disc-style candidate, but FEL use may depend on the player."
        return "Strong disc-style candidate."
    if profile_label in {"8.1", "8.4", "5"}:
        return "Good consumer-playback candidate."
    return "Playable, but not the strongest option."
...
def analyze_file(filepath: str, skip_dovi_scan: bool = False) -> dict[str, Any] | None:
    ...
    ffprobe_video = next(
        (stream for stream in ffprobe_video_streams if not stream.get("disposition", {}).get("attached_pic")),
    ...
    dovi_scan = (
        {"status": "skipped", "headline": "DV RPU scan skipped (fast mode).", "details": [], "summary": {}}
        if skip_dovi_scan
        else run_dovi_partial_scan(filepath, video_track, ffprobe_video)
    )
    ...
    bitrate_mbps = get_bitrate_mbps(video_track, ffprobe_video)
    bit_depth = get_bit_depth(video_track, ffprobe_video)

    audio_label, audio_details, audio_score = get_primary_audio_summary(mediainfo_audio_tracks, ffprobe_audio_streams)
    subtitle_count = len(get_ffprobe_streams(ffprobe_data, "subtitle"))
    source = guess_source(os.path.basename(filepath), bitrate_mbps, dv_info["base_profile"], dv_info["el"])
    ...
    tv_playback = f"{tv_support['profile_supported']} | EL {tv_support['el_usable']} | {tv_support['container_compatibility']}"
    ...
    return {
        ...
        "hdr": dv_info["hdr"],
        "bitrate_mbps": bitrate_mbps,
        "bit_depth": bit_depth if bit_depth is not None else None,
...
def scan_folder(folder: str) -> list[dict[str, Any]]:
    ...
    max_workers = min(4, max(1, len(file_paths)))

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_map = {executor.submit(analyze_file, path): path for path in file_paths}
        for future in as_completed(future_map):
            result = future.result()
            if result:
                results.append(result)

    results.sort(key=lambda item: (item["score"], item["confidence_score"], item["bitrate_mbps"]), reverse=True)
...
def save_csv(results: list[dict[str, Any]], output: str = "results.csv") -> None:
    keys = [
        ...
        "confidence_score",
        "confidence_label",
        "tv_profile_supported",
        "tv_el_usable",
        "tv_container_compatibility",
        "tv_playback_note",
        "tv_playback",
        "dv_tool",
        "quick_summary",
...
if __name__ == "__main__":
    ...
    if results:
        ...
        print("Top candidate:")
        print(f"  {results[0]['file']} | score={results[0]['score']} | confidence={results[0]['confidence_score']}")
    else:
